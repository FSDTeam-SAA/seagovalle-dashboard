...
import { ToppingsGroupedTable } from '@/components/toppings/toppings-grouped-table'
...
export function ToppingsPageContent() {
  ...
  const stats = useMemo(() => {
    const available = toppings.filter((t) => t.isAvailable).length
    const outOfStock = toppings.length - available
    return {
              ...
              initialData={
                editingTopping
                  ? {
                      ...
                      // Only include image if it's a File, otherwise omit
                      ...(typeof editingTopping.image !== 'string' && editingTopping.image
