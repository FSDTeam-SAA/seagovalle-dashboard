...
export function ToppingsPageContent() {
  ...
  const { data: analyticsData } = useGetToppingsAnalysis()
  ...
  const filteredToppings = useMemo(() => {
    if (!selectedCategory) return toppings
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Total Toppings</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold">{toppings.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
      ...
      {allCategories.length > 0 && (
        ...
        >
          <TabsList className="grid w-full max-w-3xl grid-cols-auto gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredToppings.map((topping) => (
            <ToppingCard
              ...
              onDelete={() => setDeleteConfirmId(topping.id)}
              onToggleStatus={() => handleToggleStatus(topping.id)}
            />
          ))}
        </div>
      ) : (
